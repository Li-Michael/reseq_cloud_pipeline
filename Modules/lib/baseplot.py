...
def basedis(df, name):
    """
    将百分数字符转换为float
    base = pd.read_csv('Base_distributions_by_read_position_1.txt',sep='\t')
    for i in range(len(base.iloc[0,:])):
        base.iloc[:,i] = base.iloc[:,i].str.strip('%').astype(float)
    """
    for i in range(len(base.iloc[0,:]):
        base.iloc[:,i] = base.iloc[:,i].str.strip('%').astype(float)
        
    base.columns=['%A','%T','%C','%G','%N','%clean A','%clean T', '%clean C','%clean G','%clean N']
    base = base.cumsum()
    base.plot()
    plt.savefig(name + '.png')
