...
class JsonIO(demjson.JSON):
    ...
    def json2py(self, json_s, encoding=None): # encode='utf-8'
        if os.path.isfile(json_s):
            return demjson.decode_file(json_s, encoding=encoding)
        else:
            return demjson.decode(json_s, encoding=encoding)
    ...
    def get_value(self, json_s, key=None, encoding=None):
        values = self.json2py(json_s, encoding)
        if key != None:
            value = values[key]
        else:
            value = values
