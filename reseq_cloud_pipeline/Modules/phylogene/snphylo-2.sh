...
print_help_and_exit () {
    ...
    echo 1>&2
    echo -e "Usage:" 1>&2
    echo -e "\t$(basename $0) -v VCF_file [-p Maximum_PLCS (5)] [-c Minimum_depth_of_coverage (5)]|-H HapMap_file [-p Maximum_PNSS (5)]|-s Simple_SNP_file [-p Maximum_PNSS (5)]|-d GDS_file [-l LD_threshold (0.1)] [-m MAF_threshold (0.1)] [-M Missing_rate (0.1)] [-o Outgroup_sample_name] [-P Prefix_of_output_files (snphylo.output)] [-b [-B The_number_of_bootstrap_samples (100)]] [-a The_number_of_the_last_autosome (22)] [-r] [-A] [-h]" 1>&2
    echo 1>&2
    echo -e "Options:" 1>&2
...
while getopts "v:H:d:c:p:l:m:M:P:o:s:bB:Aa:rh" OPT
do
    case "${OPT}" in
        'v')
            vcf_file="${OPTARG}"
...
then
    ...
    then
        ...
        # Generate sequences from SNP data
        "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -v "${prefix_output}.filtered.vcf" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" -a "${num_last_autosome}"
        [ $? != 0 ] && exit 1
    fi
elif [ ! -z "${hap_file}" ]
then
    ...
    then
        ...
        # Generate sequences from SNP data
        "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -H "${hap_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" -a "${num_last_autosome}"
        [ $? != 0 ] && exit 1
    else
        ...
        # Generate sequences from SNP data
        "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -H "${prefix_output}.filtered.hapmap" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" -a "${num_last_autosome}"
        [ $? != 0 ] && exit 1
    fi
elif [ ! -z "${smp_file}" ]
then
    ...
    then
        ...
        # Generate sequences from SNP data
        "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -H "${prefix_output}.hapmap" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" -a "${num_last_autosome}"
        [ $? != 0 ] && exit 1
    else
        # Remove HapMap data which have many no genotype data
        echo "Start to remove low quality data."
...
then
    ...
    # Generate sequences from SNP data
    "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -d "${gds_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" -a "${num_last_autosome}"
    [ $? != 0 ] && exit 1
else
